...
write_csv(table2_formatted, "Table_2_JMbayes_AG_Zigong.csv")
...
alpha_main_col <- grep("^value\\(aniongap\\)$", colnames(alphas_all), value = TRUE)
alpha_inter_cols <- grep("value\\(aniongap\\):ns", colnames(alphas_all), value = TRUE)
...
p_tv <- ggplot(plot_df, aes(x = Time, y = HR)) +
  ...
  labs(title = "Zigong Cohort: Time-Varying HR of Anion Gap", x = "Days since ICU Admission", y = "Hazard Ratio (95% CI)") +
  theme_classic(base_size = 14)

ggsave("Figure_2_TimeVarying_HR_AG_Zigong.pdf", p_tv, width = 8, height = 6)
